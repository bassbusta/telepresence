...
type loginExecutor struct {
	...
	SaveUserInfoFunc func(context.Context, *UserInfo) error
...
type LoginExecutor interface {
	LoginFlow(ctx context.Context, out *broadcastqueue.BroadcastQueue) error
}
...
func EnsureLoggedIn(ctx context.Context, out *broadcastqueue.BroadcastQueue) error {
	if token, _ := LoadTokenFromUserCache(ctx); token != nil {
		return nil
	}
	...
	l := NewLoginExecutor(
		...
		env.UserInfoURL,
		SaveTokenToUserCache,
		SaveUserInfoToUserCache,
		browser.OpenURL,
		client.NewScout(cmd.Context(), "cli"),
	)
	return l.LoginFlow(ctx, out)
}
...
func (l *loginExecutor) LoginFlow(ctx context.Context, out *broadcastqueue.BroadcastQueue) error {
	...
	out.Push("Launching browser authentication flow...")
	err = l.OpenURLFunc(url)
	if err != nil {
		out.Push(fmt.Sprintf("Could not open browser, please access this URL: %v", url))
	}
	...
	case callback := <-callbacks:
		...
		if err != nil {
			...
			out.Push("Login successful.")
			_ = l.retrieveUserInfo(ctx, token)
			_ = l.Scout.Report(ctx, "login_success")
		}
		return err
	case <-interrupts:
		out.Push("Login aborted.")
		_ = l.Scout.Report(ctx, "login_interrupted")
		return nil
	}
}

func (l *loginExecutor) handleCallback(ctx context.Context, callback oauth2Callback, oauth2Config oauth2.Config, pkceVerifier *CodeVerifier) (*oauth2.Token, error) {
	if callback.Error != "" {
